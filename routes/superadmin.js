...
router.get('/tenants', requireSuperadmin, async (req, res) => {
  try {
    const [rows] = await query(`
      SELECT
        t.tenant_id, t.company_name, t.slug, t.plan, t.status, t.suspension_reason, t.suspended_at, t.created_at,
      ...
      GROUP BY t.tenant_id, t.company_name, t.slug, t.plan, t.status, t.suspension_reason, t.suspended_at, t.created_at
...
router.patch('/platform-settings', requireSuperadmin, (req, res) => {
  ...
  const allowed = ['platform_name','platform_logo','primary_color','hero_title','hero_subtitle','hero_cta_text','support_email','base_domain','price_padala','price_negosyo','price_korporasyon'];
