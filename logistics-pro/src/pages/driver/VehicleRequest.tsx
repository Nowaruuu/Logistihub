...
const ITEM_TYPES = ['Package', 'Food', 'Document', 'Bulk', 'Vehicle'];
...
export default function VehicleRequestPage() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [reqRes, fleetRes] = await Promise.all([
        ...
        <>
          ...
          {view === 'list' && (
            <div className="p-4 space-y-4">
              ...
              {/* Pending staff assignment banner */}
              {pendingAssignment && (
                <div className="bg-orange-600 rounded-2xl p-5 text-white">
                  ...
                  {pendingAssignment.capacity_tons && (
                    <p className="text-white/70 text-xs mt-0.5">{pendingAssignment.capacity_tons} tons capacity</p>
                  )}
                  ...
                  <div className="flex gap-3 mt-4">
                    <button
                      onClick={() => respond(pendingAssignment.id, 'accept')}
                      disabled={submitting}
                      className="flex-1 py-2.5 bg-white text-orange-600 font-extrabold rounded-xl text-sm active:scale-95 transition-all disabled:opacity-50"
                    >
                      {submitting ? <Loader2 className="size-4 animate-spin mx-auto" /> : '✓ Accept'}
                    </button>
                    <button
                      onClick={() => { setRefuseTarget(pendingAssignment); setView('refuse'); }}
                      disabled={submitting}
                      className="flex-1 py-2.5 bg-white/20 text-white font-extrabold rounded-xl text-sm active:scale-95 transition-all border border-white/30"
                    >
                      ✕ Refuse
                    </button>
                  </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm active:scale-[0.98] transition-all"
              >
              ...
              {requests.length > 0 && (
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                  ...
                  {requests.map(req => {
                    ...
                    const Icon = cfg.icon;
                    const isAssignment = req.request_type === 'staff_assignment';
                    return (
                      <div key={req.id} className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-50 dark:border-slate-800 last:border-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">{req.vehicle_type}{req.model ? ` · ${req.model}` : ''} · {isAssignment ? 'Staff assigned' : 'Your request'}</p>
            ...
            <div className="p-4 space-y-3">
              {fleet.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <p className="text-slate-400 text-sm font-medium text-center">No fleet vehicles available right now.<br />Check back later or contact admin.</p>
                </div>
              ) : (
                fleet.map(v => {
                  ...
                  return (
                    <div key={v.plate_number} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button
                          onClick={() => requestVehicle(v.plate_number)}
                          disabled={submitting}
                          className="flex-shrink-0 px-4 py-2 bg-orange-600 text-white font-bold rounded-xl text-xs active:scale-95 transition-all disabled:opacity-50 shadow-sm shadow-orange-600/20"
                        >
          ...
          {view === 'refuse' && refuseTarget && (
            <div className="p-4 space-y-4">
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5">
                ...
                <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1">
                  <MessageSquare className="size-3" /> Reason for refusing <span className="text-red-500">*</span>
                </label>
                <textarea
                  value={refuseReason}
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <button
                    onClick={() => respond(refuseTarget.id, 'refuse', refuseReason)}
                    disabled={submitting || !refuseReason.trim()}
                    className="flex-1 py-3 bg-red-500 text-white font-bold rounded-xl text-sm active:scale-95 transition-all disabled:opacity-50"
                  >
